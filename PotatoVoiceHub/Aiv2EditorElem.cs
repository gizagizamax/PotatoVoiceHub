...
namespace PotatoVoiceHub
{
    ...
    public class Aiv2EditorElem
    {
        ...
        ///
        /// <b>これは既知のクラッシュ入力を保守的に外すための判定であって、
        /// 「Editorが読めるか」の判定ではない。</b>
        /// trueでもEditorが読めるとは限らないし(読めない文字を含んでいても通る)、
        /// falseでもEditorが読めないとは限らない(上記の「％」)。
        /// 保証しているのは「文字も数字も無いテキストは押させない」ことだけ。
        ///
        /// 走査をcharではなく文字位置で回しているのは、サロゲートペア(「𠮷」など)の
        /// ためである。charで見ると上位・下位のどちらもLetterにならず、
        /// 補助面の文字だけの文を取りこぼす。
        /// </summary>
        ...
        ///
        /// 締切が縛るのはポーリング待ちと押し直しまでで、進行中の操作は中断しない。
        /// 待ちに入る前にしか締切を見ないのでポーリング間隔(20ms)ぶん超えるほか、
        /// フォーカス安定待ち(100ms)・MSAAのCOM呼び出し・キー送出は
        /// 始まってしまえば締切に関係なく最後まで走る。
        /// 呼び出し全体の所要時間の厳密な上限ではない。
        ///
