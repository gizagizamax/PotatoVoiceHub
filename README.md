...
3.接続すると、A.I.VOICE2 Editorの右側のタブを「音声効果」へ自動で切り替えます。  
　「アクセント」タブが開いていると、Editorがセリフの文字数に応じた画面部品を大量に作るため、  
　PotatoVoiceHubからのセリフの入力・消去が極端に遅くなり、長い文章の読み上げに失敗します。  
　（実測では300文字の消去に15秒以上かかり、「音声効果」タブでは約1秒でした）  
　手動で「アクセント」タブに戻しても読み上げは続きますが、遅さで詰まった時点で再度切り替えます。  
　読み上げ中はアクセントの編集を行わないでください。  

# リリースノート  

## 💡 アップデート内容

## PotatoVoiceHub_v2026.08.11

### 🔧 長文送信時の「テキスト反映失敗・フリーズ」の修正（A.I.VOICE2対応）
* **不具合の修正**: 棒読みちゃんから長めのテキスト（目安として400〜600文字以上）を送信した際、A.I.VOICE2 Editorへのテキスト反映が失敗し、以後一切テキストが反映されなくなる致命的な連鎖不具合を修正しました。
* **原因と対策**: A.I.VOICE2の「アクセント」タブが長文時に大量の画面部品（ノード）を生成し、処理負荷が爆発的に増大することが原因でした。本バージョンより、**接続時に右ペインを負荷の低い「音声効果」タブへ自動で寄せる処理**を追加し、長文でも破綻しないよう最適化しました。
* **処理の安全化**: 一度に大量の消去キーを送信する仕様から、「32個ずつ送信して進捗を確かめる」安全なバグバスター仕様に変更し、エディタのクラッシュやキー詰まりを構造的に防ぐようにしました。
* **タイムアウト仕様の改善**: 従来の「固定の最大待ち時間」から、「1文字も処理が進まなくなってから諦めるまでの時間」に判定基準を変更しました。エディタがフリーズしない限り、どれだけ長い文章でも時間をかけて確実に反映されるようになります。
* **エラーログの強化**: 反映失敗時、どの段階（フォーカス、消去、投入）で処理が止まったのか、何文字の時点で何ミリ秒かかったのかがログへ詳細に出力されるようになり、問題の切り分けが容易になりました。

⚠️ **【仕様変更の注意点】**
不具合対策の核として、アプリ接続時にA.I.VOICE2 Editorの右ペインが自動的に「音声効果」タブに切り替わります（常時ON）。ユーザー様が手動で「アクセント」タブに戻しても読み上げは継続しますが、長文で処理が詰まった段階で自動的に「音声効果」タブへ再度押し直されます。

### 🤝 Contributors（謝辞）

本バージョンの深い原因特定、画期的なタブ切り替えによる最適化、および安全なキー送信処理の実装は、**ぽえな（@poena360）** 様にすべてご対応いただきました。
前回の修正に続き、技術的にも大変貴重な調査記録の共有、および迅速なプルリクエストのご提供に心より感謝申し上げます！

## PotatoVoiceHub_v2026.08.06

### 🔧 A.I.VOICE2 Editor 最新バージョン（2.14.1）への対応と動作安定化
* **不具合の修正**: Editorのバージョン更新（2.14.1）に伴い、UI Automation経由でのアクセシビリティ情報が取得できなくなり、`/play` および `/saveAudio` が常に `busy` を返してしまう問題を修正しました。
* **処理経路の最適化**: 内部の処理経路を `MSAA` へ置き換えることで、最新のEditor環境でも正常に動作するよう対応いたしました。
* **アクセシビリティの向上**: この変更に伴い、音声再生・保存時の**フォアグラウンド奪取（ウィンドウが強制的に最前面に出てくる挙動）が解消**され、よりバックグラウンドで快適に動作するようになりました。

### 🤝 Contributors（謝辞）

本バージョンの不具合調査、原因の特定、およびMSAAへのリプレイス実装は、**ぽえな（@poena360）** 様にすべてご対応いただきました。
貴重な調査記録の共有、および迅速なプルリクエストによるコードのご提供に、心より感謝申し上げます！

## PotatoVoiceHub_v2026.01.07
A.I.VOICE2の操作安定性を向上。  
テキスト消去の方法を `Ctrl+A` → `Ctrl+X` から `Ctrl+Z` に変更し、誤操作時の安全性を高めました。  
クリップボード監視時に無限ループ再生になるバグを修正。  
AIに内部コードのリファクタリングしてもらいました。  

## PotatoVoiceHub_v2024.03.23.2
A.I.VOICE2で文字を消す時に1ミリ秒のスリープを挟むようにしました。  

## PotatoVoiceHub_v2024.03.23
A.I.VOICE2の操作を安定化。  

## PotatoVoiceHub_v2024.03.10
A.I.VOICE2のメッセージを削除する回数を指定できるようにテキストボックスを配置。  

## PotatoVoiceHub_v2024.03.8
